...
_TIER_TOLERANCE_MB = 2048  # ±2,048 MB for current-tier matching in the reference logic
...
def _p94(values: List[float]) -> float:
    """94th-percentile of *values* using the reference implementation's cutoff."""
    if not values:
        return 0.0
    s = sorted(values)
